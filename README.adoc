...
Runs `link:https://github.com/smallrye/jandex[Jandex]` on a Gradle project.
...
Starting with version `0.14.0`, this plugin only supports Jandex 3.0+ (`io.smallrye:jandex`).
Attempts to configure an earlier Jandex version will result in a build failure.
